...
from train_relations import generate_sample, multi_generate_ARB_sample
...
class SynGenDiffusionPipeline_add_relation(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        prompt_embeds, negative_prompt_embeds = self.encode_prompt(   #[1, 77, 768]
            prompt,
            device,
        ...
        #check train or generate
        if len(data['triple']) > 0:
            prompt_embeds = multi_generate_ARB_sample(self.text_encoder, gnn_model, hete_dataset, data, self.args)
        
        # train structure information and add eot
        ...
        # 5. Prepare latent variables
            ...
            latents = self.prepare_latents(    #[8, 77, 768]
    ...
    def _syngen_step(
        ...
        with torch.enable_grad():
            latents = latents.clone().detach().requires_grad_(True)  #[4, 4, 64, 64]
            updated_latents = []
            for latent, text_embedding in zip(latents, text_embeddings):
                # Forward pass of denoising with text conditioning
                latent = latent.unsqueeze(0)  #[1, 4, 64, 64]
                ...
                loss = self._compute_loss(attention_maps=attention_maps, prompt=prompt, datas=datas)
    ...
    def _get_neg_pairs(self, pos_pairs):
        attrs = []
        objs = []
        
        for pos_pair in pos_pairs:
            
            attrs.append(pos_pair[0])
            objs.append(pos_pair[1])
        ...
        for attr in attrs:
            for obj in objs:
                if [attr, obj] in pos_pairs:
                    continue
                neg_attr_objs.append((attr, obj))

        #neg_objs
        for i in range(len(objs)-1):
            for j in range(i+1, len(objs)):
                neg_objs.append((objs[i], objs[j]))

        
        return neg_attr_objs, neg_objs

    def _compute_loss(
            self, attention_maps: List[torch.Tensor], prompt: Union[str, List[str]], datas
    ) -> torch.Tensor:
        loss = self._attribution_loss_neg_change(attention_maps,  datas)

        return loss

    def _attribution_loss_neg_change(
            self,
            attention_maps: List[torch.Tensor],
            datas,
    ) -> torch.Tensor:

        positive_pairs = datas['color_obj']

        # for key, value in datas.items():
        #     if isinstance(value, list):
        #        positive_pairs.append((value[0]+1, value[1]+1)) 
        ...
        for pos_pair in positive_pairs:
            attr, noun = pos_pair[0], pos_pair[1]
            positive_loss.append(
                calculate_positive_loss(attention_maps, attr, noun)
            )

        neg_attr_pairs, neg_objs = self._get_neg_pairs(positive_pairs)
        

        print("negative_attr_pairs:{}\n".format(neg_attr_pairs))
        print("negative_obj_pairs:{}\n".format(neg_objs))

        for neg_pair in neg_attr_pairs:
            neg_attr, neg_noun = neg_pair[0], neg_pair[1]

            negative_loss.append(
                calculate_positive_loss(attention_maps, neg_attr, neg_noun)
            )

        for neg_pair in neg_objs:
            neg_attr, neg_noun = neg_pair[0], neg_pair[1]
        ...
        loss += sum(positive_loss)/len(positive_loss)
        
        if len(negative_loss) > 0:
            loss += -sum(negative_loss)/len(negative_loss)
    ...
    def __calculate_losses_small_neg(
        self,
        attention_maps,
        all_subtree_pairs,
        subtree_indices,
        ...
        exchanged_subtree_pairs = []
        exchanged_subtree_pairs.append(all_subtree_pairs[0][0], all_subtree_pairs[1][1])
        exchanged_subtree_pairs.append(all_subtree_pairs[0][1], all_subtree_pairs[1][0])
        ...
        for neg_pair in exchanged_subtree_pairs:
            noun, modifier = neg_pair
            negative_loss.append(
                -calculate_positive_loss(attention_maps, modifier, noun)
            )
