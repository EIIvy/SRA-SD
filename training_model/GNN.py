...
def create_hg_A_ARB_B_self_loop(A, R, B, A_EOT, ARB_EOT, B_EOT, device):
    ...
    obj_features = torch.concat([A, B], dim=1)  #[2, 768]
    R_features = R    #[1, 768]
...
def create_batch_hg_A_ARB_B_self_loop(A, R, B, A_EOT, ARB_EOT, B_EOT, device):
...
def create_batch_hg_ARB_self_loop(A, R, B, A_EOT, ARB_EOT, B_EOT, device):
    ...
    for i in range(batch_size):
        ...
        ('relation', 'link_re', 'eot'): ([0], [0])})
        g = g.to(device)
        g.nodes['object'].data['feature'] = torch.concat([A[i], B[i]])  #[1, 768] ,[1, 768]
        g.nodes['relation'].data['feature'] = R[i]
        g.nodes['eot'].data['feature'] = ARB_EOT[i]
        graphs.append(g)

        obj_features = torch.concat([A[i], B[i]])  #[2, 768]
        R_features = R[i]    #[1, 768]
        EOT_features = ARB_EOT[i]  #[3, 768]
